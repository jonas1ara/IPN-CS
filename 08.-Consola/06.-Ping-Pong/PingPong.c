...
#include <time.h>	 
#include <stdlib.h>
#include <unistd.h> //Llamada al sistema sleep().
...
int main()
{
	...
	while (key != 'e' && key != 'E') // Si pulsamos E salimos del juego
	{
		key = getch();
		if (key == 'w' || key == 'W')
		{
...
void *hiloposiciones()
{
	//Ciclo for infinito para que se ejecute siempre.
	for (;;)
	{
		int i; 
		...
		// Paletas 
		if (y_paleta != 0 && y_paleta <= COLS - 1 && x_paleta != 0 && (x_paleta - 3) != LINES - 1) // Limitamos la paleta para que no se salga del marco
		{
			for (i = 0; i < 4; i++)
			{
